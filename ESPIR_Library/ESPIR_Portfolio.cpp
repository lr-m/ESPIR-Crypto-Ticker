/*
  ESPIR_Portfolio.cpp - Portfolio interface for ST7735
...
ESPIR_Portfolio::ESPIR_Portfolio(Adafruit_ST7735 *display,
                                   ESPIR_Portfolio_Editor *editor,
                                   COIN *coin_arr, ESPIR_Value_Drawer* drawer) {
  tft = display;
...
void ESPIR_Portfolio::drawValue(double *total_value, int currency) {
...
void ESPIR_Portfolio::previousMode() {
  display_mode--;
  if (display_mode < 0)
...
void ESPIR_Portfolio::drawCandleChart(double *total_value, int currency) {
  candle_graph->display(currency);
}

void ESPIR_Portfolio::drawPropBar(double *total_value) {
  double coin_total = 0;
...
// Clears the candles
void ESPIR_Portfolio::clearCandles() { candle_graph->reset(); }


// Display the coin breakdown screen with bar proportional bar chart
void ESPIR_Portfolio::drawBarSummary(double *total_value, int currency) {
...
void ESPIR_Portfolio::drawPieSummary(double *total_value) {
  double coin_total = 0;
  double current_total = 0;
  int i = 0;
...
void ESPIR_Portfolio::display(int currency) {
...
void ESPIR_Portfolio::refreshSelectedCoins() {
...
void ESPIR_Portfolio::getTotalValue(double *total_value) {
  int i = 0;
  while (portfolio_editor->selected_portfolio_indexes[i] != -1) {
    *total_value +=
        coins[portfolio_editor->selected_portfolio_indexes[i]].current_price *
...
float ESPIR_Portfolio::getFloatValue() {
  int i = 0;
  float value = 0;
  while (portfolio_editor->selected_portfolio_indexes[i] != -1) {
    value +=
...
int ESPIR_Portfolio::fillSegment(int x, int y, int startAngle, int subAngle,
