/*
  ESPIR_Portfolio_Editor.cpp - Portfolio editor interface for ST7735
...
ESPIR_Portfolio_Editor::ESPIR_Portfolio_Editor(Adafruit_ST7735 *display,
                                                 COIN *coin_arr) {
  tft = display;
...
void ESPIR_Portfolio_Editor::setActive() { active = 1; }

// Check if the amount of any coin has changed
int ESPIR_Portfolio_Editor::checkForChange() {
  if (amount_changed == 1) {
    amount_changed = 0;
    return 1;
  } else {
...
void ESPIR_Portfolio_Editor::display() {
...
void ESPIR_Portfolio_Editor::drawCoinSelected(int index) {
  tft->setTextSize(1);
...
void ESPIR_Portfolio_Editor::drawCoinUnselected(int index) {
  tft->setTextSize(1);
  tft->fillRoundRect(map(index % 3, 0, 2, 2, 2 * tft->width() / 3) + 3,
                     map(floor(index / 3), 0, 5, 5, tft->height() / 2) - 2,
...
int ESPIR_Portfolio_Editor::interact(uint32_t *ir_data) {
