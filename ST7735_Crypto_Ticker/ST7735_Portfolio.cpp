...
ST7735_Portfolio::ST7735_Portfolio(Adafruit_ST7735* display, ST7735_Portfolio_Editor* editor, COIN* coin_arr) {
	tft = display;
...
void ST7735_Portfolio::drawCandleChart(double* total_value){
	...
	while(portfolio_editor -> selected_portfolio_indexes[i] != -1){
	  ...
	  tft -> fillRect(map(current_total, 0, *total_value, 0, tft -> width()), 24, 1 + 
		map(coin_total, 0, *total_value, 0, tft -> width()), 5, 
...
// Clears the candles
void ST7735_Portfolio::clearCandles(){
	free(candle_graph);
	candle_graph = (Candle_Graph*) malloc(sizeof(candle_graph));
	*candle_graph = Candle_Graph(tft, CANDLE_COUNT, 42, tft -> height()-22, 1);
}
...
float ST7735_Portfolio::getFloatValue(){
	  ...
	  float value = 0;
	  while(portfolio_editor -> selected_portfolio_indexes[i] != -1){
		value += coins[portfolio_editor -> selected_portfolio_indexes[i]].current_price * 
		  coins[portfolio_editor -> selected_portfolio_indexes[i]].amount;
		i++;
	  }
	  return value;
...
int ST7735_Portfolio::fillSegment (int x, int y, int startAngle, int subAngle, int r, unsigned int colour)
{
